...
class PassiveSniffingWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # Kept compact (one line, no wrap) on Lite: it's an important safety
        # statement, not a workflow tip, so it stays visible — just without
        # spending as much of an 800x480 screen's vertical budget as the
        # desktop's wrapped multi-line banner did.
        ...
        # No free-text search on Lite: it's the one control here that would
        # invoke the on-screen keyboard during ordinary field use. The
        # dropdown filters below need no typing, so they're kept — see
        # _apply_filter()/_filters_are_active(), which no longer reference
        # any text query.
        filters_row = QWidget()
        filters = QGridLayout(filters_row)
        ...
        # Compact two-row arrangement of touch-friendly dropdown filters —
        # no group-box border/title around them (they're four small
        # controls, not a form worth a whole card) — plus Reset and the
        # count, all with no keyboard input required.
        ...
        # behaviorally identical to) double-clicking a row — a touchscreen
        # double-tap can be unreliable, per the task's touch-scroll
        # requirements. Double-click keeps working unchanged.
        ...
        # Touch targets: Lite's primary field-use actions get a comfortable
        # finger-sized minimum height. The persistent "Tip: double-click a
        # row..." label from the desktop build is dropped entirely rather
        # than reflowed — inspect_btn above is the discoverable, always-
        # visible replacement for it (double-click itself still works too).
        for button in (
            self.pause_btn,
            self.clear_btn,
            self.inspect_btn,
        ...
        # Lite's table strategy is scroll-not-shrink: columns keep their
        # existing readable widths (below) rather than being compressed to
        # fit 800px, and both scrollbars are explicit/pixel-accurate so a
        # finger swipe covers the rest. QScroller's TouchGesture (not
        # LeftMouseButtonGesture) only engages for actual touch input, so
        # mouse click-to-select during desktop development is unaffected.
        ...
        # width rather than the desktop build's Stretch on the last column
        # — on an 800px-wide screen the 8 columns already add up to more
        # than that (by design), so the table gets a real horizontal
        # scrollbar instead of always compressing the raw bytes to
        # whatever width happens to be left over.
        ...
        # Lets Raw RTU Frame grow to fill genuine extra width on a wide
        # window (no dead gap after the last column) without ever shrinking
        # it below the floor set above — same reasoning as the byte table
        # in packet_inspector.py.
    ...
    def _reflow_actions_row(self, *, compact: bool) -> None:
        ...
        Normal: one row, all six controls. Compact/Ultra-compact: two rows
        (Pause/Clear/Inspect, then More/Save as Profile/info icon) — no
        trailing hint label on Lite (see _build_ui), so there's nothing to
        drop here the way the desktop build's tip label was.
        """
    ...
    def inspect_selected_row(self) -> None:
        ...
        Same target (packet_inspection_requested) and behavior as
        double-clicking a row (_inspect_row) — an additional access path for
        a touchscreen where double-tap can be unreliable, per the task's
        touch-scroll requirements. Double-click keeps working unchanged.
        """
    ...
    def _apply_filter(self, *_args) -> None:
        ...
        for row in range(self.message_table.rowCount()):
            ...
            if packet is None:
                ...
                continue
            matches = (
                (direction is None or packet.direction == direction)
