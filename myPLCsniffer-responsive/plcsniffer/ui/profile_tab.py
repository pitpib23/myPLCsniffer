...
# them.
_FORMAT_COLUMN = 2
_BYTE_ORDER_COLUMN = 3
_REGISTER_COLUMN = 4
_TIMESTAMP_COLUMN = 10
_STATUS_COLUMN = 11
...
class ProfileTab(QWidget):
    ...
    def _build_table(self, parent_layout: QVBoxLayout) -> None:
        self.register_table = QTableWidget(0, 12)
        ...
        header_tooltips = {
            ...
            5: "Fills in automatically from a matching sniffed response packet.",
            6: "Scale factor applied to the raw value to produce Parsed Value.",
            7: "Fills in automatically: the raw reading × Multiplier.",
            8: "Unit label shown next to the parsed value, e.g. °C or kWh.",
            9: "Free-form notes about this register.",
    ...
    def _insert_register_row(self, register: dict) -> None:
        ...
        self.register_table.setItem(row, 5, raw_item)

        self.register_table.setItem(row, 6, QTableWidgetItem(str(register.get("multiplier", 1.0))))

        parsed_item = QTableWidgetItem(register.get("parsed_value", ""))
        parsed_item.setFlags(parsed_item.flags() & ~Qt.ItemIsEditable)
        self.register_table.setItem(row, 7, parsed_item)

        self.register_table.setItem(row, 8, QTableWidgetItem(register.get("unit", "")))
        ...
        self.register_table.setItem(row, 9, description_item)
    ...
    def _refresh_measurement_cells(self, row: int, register: dict) -> None:
        ...
        for this row.
        """
        raw_item = self.register_table.item(row, 5)
        parsed_item = self.register_table.item(row, 7)
    ...
    def _apply_filter(self) -> None:
        ...
        for row in range(self.register_table.rowCount()):
            ...
            item_description = self.register_table.item(row, 9)
