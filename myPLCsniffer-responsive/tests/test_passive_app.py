...
class PassiveSniffingWidgetTests(QtWidgetTestCase):
    ...
    def test_save_as_profile_rejects_mixed_slave_ids(self):
        widget = self.make_widget()
        try:
            widget._on_frame(response_frame(slave_id=1))
            widget._on_frame(response_frame(slave_id=2, timestamp=2.0))

            widget.save_as_profile()

            self.assertIn("only one slave ID", widget.status_label.text())
        finally:
            self.close_widget(widget)
    ...
    def test_save_as_profile_builds_registers_from_response_values(self):
        ...
        try:
            ...
            profile = captured[0]
            self.assertEqual(profile["slave_id"], 7)
            self.assertEqual(profile["start_address"], 100)
            self.assertEqual(len(profile["registers"]), 2)
            first_register, second_register = profile["registers"]
